...
def draw_landmarks(image, landmarks):
    ...
    return center(mouth_points)

class MouthPub:
    def __init__(self):
        # Create Subscriber
        self._sub = rospy.Subscriber("/camera/fake_cam_color/image_raw", Image, callback)
        # Create Publisher
        self._pub = rospy.Publisher('MouthCoordinate', Int16MultiArray, queue_size = 1)
    ...
    def callback(self, imagedata):
        ...
        if i >= 150:
            ...
            mouthcoordinate = mediapipe(cv2_array)
            self._pub.publish(mouthcoordinate)
            i = i + 1
...
if __name__ == '__main__':
    rospy.init_node('MouthNode', anonymous = True)
